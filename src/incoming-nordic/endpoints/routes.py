...
class IncomingNordic(Pipeline):
    ...
    def on_book(self):
            ...
            for root, dirs, files in os.walk(os.path.join(temp_noimages_epubdir, "EPUB")):
                for file in files:
                    ...
                    if file.endswith(".xhtml"):
                        ...
                        if not xslt.success:
                            self.utils.report.title = self.title + ": " + \
                                self.epub.identifier() + " feilet 😭👎" + epubTitle
                            return False
        ...
        archived_path, stored = self.utils.filesystem.storeBook(
            epub_fixed.asDir(), self.epub.identifier())
        self.utils.report.attachment(None, archived_path, "DEBUG")
        self.utils.report.title = self.title + ": " + \
            self.epub.identifier() + " er valid 👍😄" + epubTitle
...
def process_edition(editionId):
    ...
    process.on_book()
    # Return the report
    return return_response(process.utils.report.report())
